...
import { CounterInterpolator } from './Interpolator';
...
export class CoreWindow extends Stage {
  ...
  protected _width = 0;
  protected _height = 0;
  protected _cursorRect = new PIXI.Rectangle();
  protected _openness = 255;
  protected _animationCount = new CounterInterpolator(80);

  protected _padding = 18;
  protected _margin = 4;
  // protected _colorTone = [0, 0, 0];
  ...
  constructor() {
    ...
    this.addChild(
      ...
      this._windowCursorSprite,
      this._windowContentsSprite,
      this._downArrowSprite,
      this._upArrowSprite,
  ...
  set windowskin(value) {
    if (this._windowskin !== value) {
      ...
      value?.readyPromise.then(() => this._refreshAllParts());
    }
  }
  ...
  set contents(value: Bitmap | undefined) {
  ...
  set opacity(value) {
    this._windowSpriteContainer.alpha = numberClamp(value, 0, 255) / 255;
  }
  ...
  set backOpacity(value) {
    this._windowBackSprite.alpha = numberClamp(value, 0, 255) / 255;
  }
  ...
  set contentsOpacity(value) {
    this._windowContentsSprite.alpha = numberClamp(value, 0, 255) / 255;
  }
  ...
  set openness(value) {
    if (this._openness !== value) {
      this._openness = numberClamp(value, 0, 255);
  ...
  public override updateDelta(delta: number) {
    if (this.active) {
      this._animationCount.updateDelta(delta);
    }
   ...
   */
  public addChildToBack(child: Stage) {
  ...
  /**
   * Refresh the mask. Originally implemented in WindowLayer.
  ...
  /**
   * In PIXI 8 this is not called regularly
  ...
  protected _updateCursor() {
    const blinkCount = this._animationCount.value % 40;
  ...
  protected _updatePauseSign() {
    ...
    const x = Math.floor(this._animationCount.value / 16) % 2;
    const y = Math.floor(this._animationCount.value / 16 / 2) % 2;
    const sx = 144;
    const sy = 96;
    const p = 24;
    if (!this.pause) {

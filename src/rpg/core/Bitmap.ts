...
export class Bitmap {
  ...
  protected __context: CanvasRenderingContext2D | undefined;
  protected __baseTexture: PIXI.TextureSource | undefined;
  protected _dirty: boolean = false;

  protected _errorListener: (() => void) | undefined;
  protected _loadListener: (() => void) | undefined;
  ...
  constructor(width: number = 0, height: number = 0, defer: boolean = false) {
    if (!defer) {
      this._createCanvas(width, height);
    }
  }

  protected _createCanvas(width = 0, height = 0) {
  ...
  protected _createBaseTexture(source?: HTMLCanvasElement | ImageBitmap) {
    ...
    this.__baseTexture = new PIXI.ImageSource({
      ...
      style: { scaleMode: this._smooth ? 'linear' : 'nearest' },
    });
  }

  protected _clearImgInstance() {
    if (!this._image) {
      return;
    }
    // this._image.src = '';
   ...
   * @return Bitmap
   */
  public static load(url: string, smooth?: boolean) {
  ...
  public getPixel(x: number, y: number) {
    ...
    let result = '#';
    for (let i = 0; i < 3; i++) {
      result += stringPadZero(data[i].toString(16), 2);
    }
    return result;
  }
   ...
   * @private
   */
  protected _drawTextOutline(text: string, tx: number, ty: number, maxWidth: number) {
    const context = this._context;
   ...
   * @private
   */
  protected _drawTextBody(text: string, tx: number, ty: number, maxWidth: number) {
    const context = this._context;
  ...
  public async decode() {
    if (this._loadingState === 'requestCompleted' || this._loadingState === 'decryptCompleted') {
      this._loadingState = BitmapLoadingState.Loaded;
  ...
  public static request(url: string, smooth?: boolean) {
    const bitmap = new Bitmap(undefined, undefined, true);
    if (smooth) {
      // True only
    ...
    bitmap._url = url;
    bitmap._loadingState = BitmapLoadingState.Pending;

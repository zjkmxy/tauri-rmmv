...
import { numberClamp } from '../core/JsExtensions';
...
/** The window class with cursor movement and scroll functions. */
export abstract class Window_Selectable<Key extends string> extends Window_Base {
  protected _index = -1;
  ...
  protected initialize(x?: number, y?: number, width?: number, height?: number) {
  ...
  public get cursorFixed() {
    return this._cursorFixed;
  }

  public set cursorFixed(cursorFixed: boolean) {
    this._cursorFixed = cursorFixed;
  }

  public get cursorAll() {
    return this._cursorAll;
  }

  public set cursorAll(cursorAll: boolean) {
  ...
  public setTopRow(row: number) {
    const scrollY = numberClamp(row, 0, this.maxTopRow()) * this.itemHeight();
  ...
  public cursorDown(wrap: boolean = false) {
  ...
  public cursorUp(wrap: boolean = false) {
    const index = this.index();
    const maxItems = this.maxItems();
    const maxCols = this.maxCols();
  ...
  public abstract drawItem(index: number): void;

  public clearItem(index: number) {
    const rect = this.itemRect(index);

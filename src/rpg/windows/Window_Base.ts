...
export class Window_Base extends CoreWindow {
  ...
  static readonly _iconWidth = 32;
  static readonly _iconHeight = 32;
  static readonly _faceWidth = 144;
  static readonly _faceHeight = 144;

  protected _openingResolver: (() => void) | undefined;
  protected _closingResolver: (() => void) | undefined;
  ...
  public updateOpen() {
    if (this._opening) {
      ...
      if (this.isOpen()) {
        this._opening = false;
        this._openingResolver?.();
        this._openingResolver = undefined;
      }
    }
  }
  ...
  public updateClose() {
    if (this._closing) {
      ...
      if (this.isClosed()) {
        ...
        this._closingResolver = undefined;
      }
    }
  }
  ...
  public convertEscapeCharacters(text: string) {
    ...
    text = text.replace(/\\N\[(\d+)\]/gi, (_, p1) => {
      // return this.actorName(parseInt(p1));
      return p1;
    });
    text = text.replace(/\\P\[(\d+)\]/gi, (_, p1) => {
      // return this.partyMemberName(parseInt(p1));
      return p1;
    });
  ...
  public processNewPage(textState: TextState) {
    textState.index++;
  }
  ...
  public makeFontBigger() {
    if (this.contents && this.contents.fontSize <= 96) {
      this.contents.fontSize += 12;
    }
  }

  public makeFontSmaller() {
    if (this.contents && this.contents.fontSize >= 24) {
      this.contents.fontSize -= 12;
    }
  }
  ...
  // public drawActorName(actor, x, y, width) {
  //   width = width || 168;
  //   this.changeTextColor(this.hpColor(actor));
  //   this.drawText(actor.name(), x, y, width);
  // };

  // public drawActorClass(actor, x, y, width) {
  //   width = width || 168;
  //   this.resetTextColor();
  ...
  // public drawActorIcons(actor, x, y, width) {
  //   width = width || 144;
  ...
  public refreshDimmerBitmap() {
    if (this._dimmerSprite && this._dimmerSprite.bitmap) {
      const bitmap = this._dimmerSprite.bitmap;
      const w = this.width;
      const h = this.height;
  ...
  public canvasToLocal({ x, y }: { x: number; y: number }) {
    for (let node = this as PIXI.Container; node; node = node.parent) {
      x -= node.x;
      y -= node.y;
    }
    return { x, y };
  ...
  public canvasToLocalX(x: number) {
    return this.canvasToLocal({ x: x, y: 0 }).x;
  }

  public canvasToLocalY(y: number) {
    return this.canvasToLocal({ x: 0, y: y }).y;
  }

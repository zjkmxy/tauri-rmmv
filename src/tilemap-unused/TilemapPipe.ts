import { BatchableMesh, ExtensionType, InstructionSet, MeshGeometry, Renderer, RenderPipe } from 'pixi.js';
...
export class TilemapPipe implements RenderPipe<Tilemap> {
  ...
  public addRenderable(renderable: Tilemap, _instructionSet: InstructionSet) {
    ...
    if (canBatch) {
      ...
      if (renderable._didTilemapUpdate) {
        ...
        batchableMesh.mesh = renderable;
        batchableMesh.texture = renderable.baseTexture;
      }
  ...
  validateRenderable(renderable: Tilemap): boolean {
    ...
    if (batchableMesh?.texture._source !== renderable.baseTexture._source) {
      return !batchableMesh?.batcher.checkAndUpdateTexture(batchableMesh, renderable.baseTexture);
    }
